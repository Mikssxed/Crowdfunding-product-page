if (localStorage.getItem('width') != null) { //takes saved data, allows to reload
    document.querySelectorAll('.leftRewardBamboo')[0].innerHTML = localStorage.getItem('leftBambooFinal0');
    document.querySelectorAll('.leftRewardBamboo')[1].innerHTML = localStorage.getItem('leftBambooFinal1');
    document.querySelectorAll('.leftRewardBlack')[0].innerHTML = localStorage.getItem('leftBlackFinal0');
    document.querySelectorAll('.leftRewardBlack')[1].innerHTML = localStorage.getItem('leftBlackFinal1');
    document.getElementById('paid').innerHTML = localStorage.getItem('moneyFinal');
    document.getElementById('beckers').innerHTML = localStorage.getItem('backerFinal');
    document.getElementById('fillBar').style.width = localStorage.getItem('width');
}
if (localStorage.getItem('bookmarkFinal') != null) {
    document.getElementById('bookmark').innerHTML = localStorage.getItem('bookmarkFinal');
    if (document.getElementById('bookmark').innerHTML != `<img src="./images/icon-bookmark.svg">Bookmark`) {
...
bookmark.addEventListener('click', function bookmarkFnc() { //adds or removes ed to bookmark button and also changes color
    ...
    } else if (bookmark.classList.contains('bookmarked') == false) { //if false "deletes" ed from end
...
hide.addEventListener('click', function close() { //hides modal
...
const Select = () => { //toggle selectors
    for (let i = 0; i < 3; i++) { //iterates through every chooose and if it have said class it appear or disappear
...
choose.forEach(item => { //adds class selectedFor
...
for (let i = 0; i < paidValue.length; i++) { //takes numbers from paid value
    if(paidValue[i] != "," && paidValue[i] != "$") {
       result += paidValue[i];
    }
}

for (let i = 0; i < backersValue.length; i++) { //takes numbers from backers
    if(backersValue[i] != ",") {
       left += backersValue[i];
    }
}
...
const progressBar = () => {
    progress.style.width = `${result / 1000}%` //calculates width of progress bar
}

const addToMoney = (nr) => {
    result = Number(result) + Number(money[nr].value); //adds amount of number from input and sets it to variable
    let firstHalf = '';
    let secondHalf = '';
    if (result.toString().length == 5) { //slices string of exact length to add "," between sliced elements
...
const addToBackers = () => {
    left = Number(left) + 1 //add number of backers
...
const getMoney = (nr) => { //takes actual value and adds from input
    addToMoney(nr);
    const hide = () => {
        modal.style.display = "none"
    }
...
const leftDown = (nr) => { //decrease number of items left
    if (nr == 1) {
...
succesBtn.addEventListener('click', function close() { //hides
